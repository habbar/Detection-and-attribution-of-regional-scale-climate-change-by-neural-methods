...
Contains inverting functions:
    ...
    #                            verbose =False,
    #                           ) :
Created on Wed Apr 24 17:05:27 2023
...
filter_lanc = torch.tensor([-2.942787e-10, 0.004376876, 0.01252928, 0.0196969, 0.01802997, 2.067801e-09,
                            -0.03728088, -0.08909709, -0.1434092, -0.1847471, 0.7998024, -0.1847471,
                            -0.1434092, -0.08909709, -0.03728088, 2.067801e-09,
                            0.01802997, 0.0196969, 0.01252928, 0.004376876, -2.942787e-10],
                           requires_grad=True).unsqueeze(0).unsqueeze(0)

class Net_regul(nn.Module):

    def __init__(self, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        super(Net_regul, self).__init__()

        self.device = device
        self.conv = nn.Conv1d(1,1,kernel_size=21)
        self.conv.weight = nn.parameter.Parameter(filter_lanc)
    ...
    def forward(self, X):
        ...
        L = X.shape[2]
        x = torch.zeros((X.shape[0],X.shape[1],L+20)).to(device)
        mean = torch.mean(X[:,:,0:10],2).to(device)
        debut = torch.stack((mean, mean, mean, mean, mean, mean, mean, mean, mean, mean),dim = 2).to(device)
        fin = torch.stack((2 * X[:, :, L-1] - X[:, :, L-2],
                           2 * X[:, :, L-1] - X[:, :, L-3],
                           2 * X[:, :, L-1] - X[:, :, L-4],
                           2 * X[:, :, L-1] - X[:, :, L-5],
                           2 * X[:, :, L-1] - X[:, :, L-6],
        ...
        x1 = torch.std(self.conv(x[:,0].unsqueeze(1)))
        x2 = torch.std(self.conv(x[:,1].unsqueeze(1)))
        x3 = torch.std(self.conv(x[:,2].unsqueeze(1)))
...
def model_cnn_inverse(Xo, X_rappel, Y, CNN, dev, beta=0.1, lr_opt=0.01, alpha=0.1, n_iter=200, regula=None,
                      lr_reg=None,   # other name for BETA, for backward compatibility. If not None replaces the value of BETA
    ...
    if regula is not None :
        model_regul = Net_regul(device=dev)
        #model_regul = Net_regul(device=dev).to(dev)
        do_regutation = True
    ...
    for i in range(n_iter):
        ...
        loss_model = criterion(Y_hat, Y)
        loss_rappel = criterion(X, X_rappel)
        
        #loss = (1 - beta)*loss_model + beta * loss_rappel
        loss = loss_model + beta * loss_rappel

        if do_regutation :
            loss_regul = model_regul(X)
            loss += regula * loss_regul
        loss = torch.sqrt(loss)
        
        if loss_trace :
            if not do_regutation :
                print('inversion:',i+1,loss_model,loss_rappel,loss)
            else:
                print('inversion:',i+1,loss_model,loss_rappel,loss_regul,loss)
        
        #loss = criterion(Y_hat,Y) + beta * criterion(X,X_rappel)
        ...
        if ret_loss0et1:
            loss0_list.append(loss_model.cpu().detach().clone().numpy().tolist())
            loss1_list.append(loss_rappel.cpu().detach().clone().numpy().tolist())

        delta_loss = np.abs(loss.cpu().detach().clone().numpy() - loss_back)
        if delta_loss < delta_loss_limit :
            patience_count += 1
        
        if loss < alpha or (delta_loss_limit is not None and delta_loss < delta_loss_limit and patience_count > patience) :
            if loss < alpha*alpha :
                convergence_flg = True

            if verbose :
                _tmp_loss = loss.cpu().detach().clone().numpy().tolist()
                print(f" ** Stopping at iteration {i+1}/{n_iter}, last loss={_tmp_loss}, delta loss={delta_loss}")

                if loss < alpha :
                    print(f" ** Stopping condition observed:  << CONVERGENCE ACHIEVED >>\n **   loss < alpha: {_tmp_loss} < {alpha}")
                elif delta_loss_limit is not None and delta_loss < delta_loss_limit and patience_count > patience:
                    print(f" ** Stopping condition observed: << CONVERGENCE NOT ACHIEVED >>:\n **   delta_loss < delta_loss_limit: {delta_loss} < {delta_loss_limit} (patience: {patience})")
                else:
                    print(f" ** STOPPING REASON NOT CLEAR BUT STOP ANY WAY (!?) ... BIZARRE :\n **   Loss: {_tmp_loss}, alpha: {alpha}, delta_loss: {delta_loss}, delta_loss_limit: {delta_loss_limit}, patience: {patience}")
    ...
    if verbose :
        if convergence_flg :
            print(f" -- CONVERGENCE ACHIEVED at {i+1} iterations.")
        else:
            print(f" ** CONVERGENCE NOT ACHIEVED in {i+1} iterations.")
    
    if ret_loss0et1 :
        return Y_hat,X,i+1,loss_list,loss0_list,loss1_list
    elif ret_loss :
        return Y_hat,X,i+1,loss_list
    else:
        return Y_hat,X,i+1
...
def inversion_procedure (base_case_to_explore, sub_case_to_explore, inversion_suffix, do_inversion=False,
                         ...
                         inv_n_iter=2000, regula = None, inv_lr_reg=0.1, inv_lr_opt=0.01, inv_alpha=0.1, inv_delta_loss_limit=1e-4, inv_delta_limit_patience=10,
                         train_years=np.arange(1900,2015), 
                         lp_nathist_filtering_dic_file='lp_nat_and_hist_filtering_param_dictionary.p',
    ...
    inv_label = f'Lr{inv_lr_reg}-Opt{inv_lr_opt}-A{inv_alpha}'
    if regula is not None:
        inv_label += f'-regu{regula}'
        
    inv_label += f'_{inversion_suffix}'
...
def write_rmse_to_file(rmse_file_name, forc, rmse1, rmse2, rmse3):
    with open(rmse_file_name, 'a') as f:
        f.write(f'{forc},{rmse1},{rmse2},{rmse3}\n')
...
def plot_averaged_inv_all_forcings_by_net (base_case_to_explore, sub_case_to_explore, inversion_suffix,
                                           ...
                                           models_to_plot=None, load_best_val=False, load_best_val2nd=False,
                                           plot_src4inv=False, plot_inv_x=True, plot_inv_filt_x = False, plot_ini_x=True,
                                           ...
                                           plot_src4inv_forc_shaded_region=False,
                                           plot_src4inv_shaded_region = False,
                                           plot_src4inv_hist = False,
                                           plot_src4inv_shaded_hist = False,
                                           plot_inv_forc_shaded_region=False,
                                           plot_ini_forc_shaded_region = False,
                                           plot_inv_filt_forc_shaded_region = False,
                                           plot_hist2inv = False,
                                           plot_mean_other_mod = False,
                                           plot_hist2inv_shaded_region = False, hatch_for_hist2inv_shaded='|',
                                           plot_hist_shaded_region = False, 
                                           plot_inv_hist_shaded_region = False,
                                           ...
                                           alpha_for_mean_shaded=0.2, lw_for_mean_shaded=2.5, hatch_for_mean_shaded_inv='||',
                                           force_plot=False, force_write=False, plot_mean_err=False, mean_err_lightness_factor=0.25,
                                           inv_label=None, t_limits=None,
                                           train_years=np.arange(1900,2015),
                                           rmse_start_year = 1960,
                                           local_nb_label="PlotAveInvAllForcfByNet", fig_ext='png',
                                           figs_defaults={'dpi':300, 'facecolor':'w', 'edgecolor':'w'},
                                           #obs_filename = 'obs.npy', obs_name = 'OBS',
                                           obs_set=['OBS','HadCRUT','HadCRUT200','HadCRUT+AR1'],
    ...
    inversion_years = train_years
    lenDS_4_rms = inversion_years[-1] - rmse_start_year + 1
    ...
    for innet in np.arange(n_nnets):
        ...
        if case_figs_dir is not None and not force_plot and save_figs and os.path.isfile(figs_filename):
            ...
            for i_mod,(mod_to_invert,mod_data_source,mod_sdir) in enumerate(zip(inverted_model_to_plot,source_model_data_to_plot,models_sdirs_to_plot)) : 
                ...
                if one_ax_by_forcing and n_models_to_plot_in_one_page > 1 :
                    print('cannot plot one by forcing')
                    one_ax_by_forcing = False
                    
                if one_ax_by_forcing :
                    ncols = 2
                ...
                if one_ax_by_forcing :
                    nrows = 2 # int(np.ceil(n_models_to_plot_in_one_page/ncols))
                else:
                ...
                for i_trmod,mod_from_training in enumerate(models_from_training) :
                    ...
                    if case_figs_dir is None :
                        ...
                        case_figs_dir_csv = os.path.join(figs_dir, base_case_to_explore, cnn_name_base, f'Settings-{inv_label}',mod_to_invert)
                        print(f'Repertoire des csv du cas: {case_figs_dir_csv}')
                    ...
                    if innet == 0:
                        ...
                        # un print pour voir de quel modèle vient chaque profil des multiobs (pour chaque modèle)  
                        for i in range(len(Nbe_list)):
                            print(f"  Nbe_list[{i}] = {Nbe_list[i]}")
                    ...
                    if plot_src4inv :
                        if GHG_mod_for_inv_array is None :
                            ...
                            for iforc,(forc,cs) in enumerate(zip(forcing_names,forcing_src_colors)) :
                                ...
                                if forc == 'ghg':
                                    FORC_src_from_model = GHG_mod_for_inv_array
                                    local_plot_forc_shaded_region = plot_src4inv_forc_shaded_region
                                elif forc == 'aer':
                                    FORC_src_from_model = AER_mod_for_inv_array
                                    local_plot_forc_shaded_region = plot_src4inv_forc_shaded_region
                                elif forc == 'nat':
                                    ...
                                    if plot_src4inv_hist:
                                        FORC_src_from_model = HIST_mod_for_inv_array 
                                        local_plot_forc_shaded_region = plot_src4inv_shaded_hist
                                    else:
                                        FORC_src_from_model = None
                                else:
                                    FORC_src_from_model = None
                                ...
                                if FORC_src_from_model is not None:
                                    ...
                                    if local_plot_forc_shaded_region :
                                        if current_std_FORC_m is not None and plot_src4inv_shaded_region :
                    ...
                    if plot_ini_x :
                        for iforc,(forc,cs) in enumerate(zip(forcing_names[:3],forcing_colors[:3])) :
                            ...
                            if FORC_ini_for_inv_array is not None:
                                ...
                                if plot_forc_shaded_region and False :
                                    if plot_ini_forc_shaded_region :
                    ...
                    if plot_inv_x :
                        ...
                        if len(Nbe_list) > 1:
                            ...
                            print(f"Calcul de RMSE pour chaque forçage à partir de {rmse_start_year} à 2014 entre le forçage moyen des données source du modèle {mod_data_source} et le forçage moyen des données inversées ({Xinv_arr.shape[1]} forçages).")
                            print(f"Calcul de RMSE pour chaque forçage à partir de {rmse_start_year} à 2014 entre le forçage moyen des données des autres modèles et le forçage moyen des données inversées ({Xinv_arr.shape[1]} forçages).")
                            print(f"Calcul de RMSE pour chaque forçage à partir de {rmse_start_year} à 2014 entre le forçage moyen des données source du modèle {mod_data_source} et le forçage moyen des données des autres modèles.")
                            ...
                            for iforc,(forc,cs) in enumerate(zip(forcing_names[:3],forcing_colors[:3])) :
                                if one_ax_by_forcing :
                                    ...
                                    ax = axes[iforc]
                                if forc == 'ghg':
                                    FORC_inv_array = Xinv_arr[:,:,0,:].mean(axis=0) 
                                    FORC_src_from_model = GHG_mod_for_inv_array
                                    FORC_mean_oth_mod = GHG_ens_ab_arr_m
                                elif forc == 'aer':
                                    FORC_inv_array = Xinv_arr[:,:,1,:].mean(axis=0)
                                    FORC_src_from_model = AER_mod_for_inv_array
                                    FORC_mean_oth_mod = AER_ens_ab_arr_m
                                elif forc == 'nat':
                                    FORC_inv_array = Xinv_arr[:,:,2,:].mean(axis=0)
                                    FORC_src_from_model = NAT_mod_for_inv_array
                                    FORC_mean_oth_mod = NAT_ens_ab_arr_m
                                else:
                                    FORC_inv_array = None
                                ...
                                if FORC_inv_array is not None:
                                    ...
                                    rmse1 = gt.RMSE_mean(FORC_inv_array, FORC_src_from_model, n_end = lenDS_4_rms)
                                    print(f'RMSE for {forc} Xinv / X_src: {rmse1}')
                                    
                                    rmse2 = gt.RMSE_mean(FORC_inv_array, FORC_mean_oth_mod, n_end = lenDS_4_rms)
                                    print(f'RMSE for {forc} Xinv / X_other_mod: {rmse2}')
                                    
                                    rmse3 = gt.RMSE_mean(FORC_src_from_model, FORC_mean_oth_mod, n_end = lenDS_4_rms)
                                    print(f'RMSE for {forc} X_src / X_other_mod: {rmse3}')
                                    
                                    all_rmse_results.append((forc, FORC_inv_array.shape[0], rmse1, rmse2, rmse3))
                                    ...
                                    FORC_src_from_model_mean = FORC_src_from_model.mean(axis=0)
                                    print(f"Calcul de la distance pour chaque forçage à partir de {rmse_start_year} à 2014 entre le forçage moyen des données source du modèle {mod_data_source} et le forçage moyen des données inversées ({Xinv_arr.shape[1]} forçages).")
                                    dist_mod_inv_src = FORC_src_from_model_mean - current_meanFORC_m
                                    rmse_mod_inv_src = gt.RMSE_mean(FORC_src_from_model_mean, current_meanFORC_m, n_end = lenDS_4_rms)
                                    # .npy os.path.join(case_figs_dir_csv, f'RMSE_{inv_label}_Xinv-Xsrc-Xothmod_{forc}.csv')
                                    np.save(os.path.join(case_figs_dir, f'dist_{inv_label}_Xinv-Xsrc_{forc}.npy'), dist_mod_inv_src)
                                    np.save(os.path.join(case_figs_dir, f'rmse_{inv_label}_Xinv-Xsrc_{forc}.npy'), rmse_mod_inv_src)
                                    # afficher ces distances sur un graphique avec les RMSE en titre
                                    figur, axie = plt.subplots(1,1, figsize=(8,6))
                                    axie.plot(current_inv_years, dist_mod_inv_src, color=cs, lw=1.)
                                    axie.set_title(f"RMSE {rmse_mod_inv_src:.2f} {forc.upper()} Xinv-Xsrc")
                                    axie.set_ylabel(f"{forc.upper()} dist (W.m-2)")
                                    axie.set_xlabel("year")
                                    axie.grid(True)
                                    axie.set_xlim(rmse_start_year, 2014)
                                    axie.set_ylim(-1.5, 1.5)
                                    axie.axhline(0., color='k', lw=1.)
                                    plt.savefig(os.path.join(case_figs_dir, f'dist_{inv_label}_Xinv-Xsrc_{forc}.png'), dpi=300, bbox_inches='tight')
                                    plt.close(figur)
                                    ...
                                    if plot_forc_shaded_region :    
                                        ...
                                        ax.plot(current_inv_years, current_meanFORC_m, color=cs, lw=1.5, ls='-', label=f"inv {forc.upper()}")
                            ...
                            rmse_file_name_global = os.path.join(case_figs_dir, f'RMSE_{inv_label}_Xinv-Xsrc-Xothmod.csv')
                            print(f"rmse_file_name_global: {rmse_file_name_global}")
                            if not os.path.exists(rmse_file_name_global):
                                with open(rmse_file_name_global, 'w') as f_global:
                                    f_global.write("Forcing,RMSE Xinv/X_src,RMSE Xinv/X_other_mod,RMSE X_src/X_other_mod\n")

                            # Parcourir la liste temporaire et écrire les résultats dans les fichiers CSV spécifiques à chaque forçage
                            for res in all_rmse_results:
                                forc, nb_inverted_points, rmse1, rmse2, rmse3 = res
                                rmse_file_name = os.path.join(case_figs_dir_csv, f'RMSE_{inv_label}_Xinv-Xsrc-Xothmod_{forc}.csv')
                                print(f"rmse_file_name: {rmse_file_name}")

                                if not os.path.exists(rmse_file_name):
                                    with open(rmse_file_name, 'w') as f:
                                        f.write("Nb_inverted_points,RMSE Xinv/X_src,RMSE Xinv/X_other_mod,RMSE X_src/X_other_mod\n")
                                        f.write(f'{nb_inverted_points},{rmse1},{rmse2},{rmse3}\n')

                                else:
                                    with open(rmse_file_name, 'r') as f:
                                        lines = f.readlines()
                                        last_line = lines[-1].strip()  # Get the last line (remove newline characters)
                                        last_values = last_line.split(',')  # Split the last line by commas

                                        # Check if the last line has the same values as the current results
                                        if len(last_values) == 4 and \
                                        int(last_values[0]) == nb_inverted_points and \
                                        float(last_values[1]) == rmse1 and \
                                        float(last_values[2]) == rmse2 and \
                                        float(last_values[3]) == rmse3:
                                            print(f"Data for {forc} with {nb_inverted_points} inverted points already exists. Skipping...")
                                        else:
                                            # If the last line does not have the same values, write the results
                                            with open(rmse_file_name, 'a') as f:
                                                f.write(f'{nb_inverted_points},{rmse1},{rmse2},{rmse3}\n')

                                with open(rmse_file_name_global, 'a') as f_global:
                                    f_global.write(f'{forc},{rmse1},{rmse2},{rmse3}\n')
                                                        
                            # plotter ça pour chaque fichier il me fait 3 subplots un pour ghg un pour nat et un pour aer sur l'axe des abscices c'est les nombres de points de départ et dans les ordonnées c'est les différentes RMSE avec des labels et avec un titre pour les trois figures
                            import matplotlib.pyplot as plt
                            import pandas as pd
                            
                            # Calculate the global minimum and maximum RMSE values across all CSV files for all three cases
                            global_min_rmse_xinv_xsrc = np.inf
                            global_max_rmse_xinv_xsrc = -np.inf
                            global_min_rmse_xinv_xother = np.inf
                            global_max_rmse_xinv_xother = -np.inf
                            global_min_rmse_xsrc_xother = np.inf
                            global_max_rmse_xsrc_xother = -np.inf

                            for forc in ['ghg', 'aer', 'nat']:
                                rmse_file_name = os.path.join(case_figs_dir_csv, f'RMSE_{inv_label}_Xinv-Xsrc-Xothmod_{forc}.csv')
                                if os.path.exists(rmse_file_name):
                                    df = pd.read_csv(rmse_file_name)
                                    
                                    # Update global minimum and maximum for Xinv/X_src
                                    global_min_rmse_xinv_xsrc = min(global_min_rmse_xinv_xsrc, df['RMSE Xinv/X_src'].min())
                                    global_max_rmse_xinv_xsrc = max(global_max_rmse_xinv_xsrc, df['RMSE Xinv/X_src'].max())
                                    
                                    # Update global minimum and maximum for Xinv/X_other_mod
                                    global_min_rmse_xinv_xother = min(global_min_rmse_xinv_xother, df['RMSE Xinv/X_other_mod'].min())
                                    global_max_rmse_xinv_xother = max(global_max_rmse_xinv_xother, df['RMSE Xinv/X_other_mod'].max())
                                    
                                    # Update global minimum and maximum for X_src/X_other_mod
                                    global_min_rmse_xsrc_xother = min(global_min_rmse_xsrc_xother, df['RMSE X_src/X_other_mod'].min())
                                    global_max_rmse_xsrc_xother = max(global_max_rmse_xsrc_xother, df['RMSE X_src/X_other_mod'].max())

                            # Plotting the RMSE for each forcing with adjusted y-axis scale
                            figu, axies = plt.subplots(1, 3, figsize=(20, 5))
                            figu.suptitle(f"RMSE for backward optimization for different number of initial conditions - ({inv_label}) - {mod_to_invert}", fontsize=16)
                            figu.subplots_adjust(top=0.85)

                            for i, forc in enumerate(['ghg', 'aer', 'nat']):
                                rmse_file_name = os.path.join(case_figs_dir_csv, f'RMSE_{inv_label}_Xinv-Xsrc-Xothmod_{forc}.csv')
                                if os.path.exists(rmse_file_name):
                                    df = pd.read_csv(rmse_file_name)

                                    axies[i].plot(df['Nb_inverted_points'], df['RMSE Xinv/X_src'], label=f"Xinv_inv_{forc.upper()} / X_src_{forc.upper()}", marker='o', linestyle='-')
                                    axies[i].plot(df['Nb_inverted_points'], df['RMSE Xinv/X_other_mod'], label=f"Xinv_inv_{forc.upper()} / X_other_mod_{forc.upper()}", marker='o', linestyle='-.')
                                    axies[i].plot(df['Nb_inverted_points'], df['RMSE X_src/X_other_mod'], label=f"X_src_{forc.upper()} / X_other_mod_{forc.upper()}", linestyle='-')

                                    axies[i].set_title(forc.upper())
                                    axies[i].grid(True)
                                    axies[i].set_xlabel("Nb inverted points")

                                    # Set y-axis limits to be the same for all subplots for each case
                                    axies[i].set_ylim(min(global_min_rmse_xinv_xsrc, global_min_rmse_xinv_xother, global_min_rmse_xsrc_xother)-0.02,
                                                    max(global_max_rmse_xinv_xsrc, global_max_rmse_xinv_xother, global_max_rmse_xsrc_xother)+0.02)

                                    if i == 0:
                                        axies[i].set_ylabel("RMSE")

                                    if i > 0:
                                        axies[i].set_yticklabels([])

                                    #axies[i].legend()
                                    figu.tight_layout()

                            figu.savefig(os.path.join(case_figs_dir, f'RMSE_{inv_label}_Xinv-Xsrc-Xothmod.png'))
                            figu.show()
                            ...
                            # Create the figure and axes for the distances
                            bottom = 0.08; top = 0.85
                            left   = 0.10; right  = 0.98
                            wspace = 0.05; hspace = 0.12
    
                            fig_dist, ax_dist = plt.subplots(nrows=1, ncols=1, figsize=(9.5, 7.), gridspec_kw={'hspace': hspace, 'wspace': wspace,
                                                                                'left': left, 'right': right,
                                                                                'top': top, 'bottom': bottom })
                            
                            #ax_dist.set_title(f"Distance between {mod_data_source} data and inverted data - {inv_label}")
                            
                            #ax_dist.set_ylabel("Distance (°C)")
                            #ax_dist.set_xlabel("Years")
                            ax_dist.grid(True)
                            #ax_dist.axhline(0., color='k', lw=1.)
                            ...
                            for iforc,(forc,cs) in enumerate(zip(forcing_names[:3],forcing_colors[:3])) :
                                ...
                                if FORC_inv_array is not None:
                                    ...
                                    all_rmse_results.append((forc, FORC_inv_array.shape[0], rmse1, rmse2, rmse3))
                                    '''
                                    # Chemin du fichier CSV pour ce forçage
                                    rmse_file_name = os.path.join(case_figs_dir_csv, f'RMSE_{inv_label}_Xinv-Xsrc-Xothmod_{forc}.csv')
                                    print(f"rmse_file_name: {rmse_file_name}")
                                
                                    # Écrire les en-têtes dans le fichier CSV spécifique à ce forçage
                                    with open(rmse_file_name, 'w') as f:
                                        f.write("Nb_inverted_points,RMSE Xinv/X_src,RMSE Xinv/X_other_mod,RMSE X_src/X_other_mod\n")

                                        # Écrire les résultats dans le fichier CSV spécifique à ce forçage
                                        f.write(f'{FORC_inv_array.shape[0]},{rmse1},{rmse2},{rmse3}\n')
                                    ...
                                    np.save(os.path.join(case_figs_dir, f'dist_{inv_label}_Xinv-Xsrc_{forc}.npy'), dist_mod_inv_src)
                                    np.save(os.path.join(case_figs_dir, f'rmse_{inv_label}_Xinv-Xsrc_{forc}.npy'), rmse_mod_inv_src)
                                    ax_dist.plot(current_inv_years, dist_mod_inv_src, color=cs, label=f"RMSE {forc.upper()} mean: {rmse_mod_inv_src:.2f}")
                                    ax_dist.set_ylim(-3.,2.5,1)
                                        ...
                                        if plot_inv_forc_shaded_region :                                        
                                            if current_std_FORC_m is not None:
                                                ...
                                                ax.fill(np.NaN, np.NaN, 
                                                        ec=cs, fc=gt.lighter_color(cs), alpha=alpha_for_mean_shaded, linewidth=lw_for_mean_shaded,
                                                        hatch=hatch_for_mean_shaded_inv, label=f"inv {forc.upper()}")
                                        ax.plot(current_inv_years, current_meanFORC_m, color=cs, lw=2.5, ls='-', label=f"inv {forc.upper()}")
                            ...
                            # Add a legend to the distance plot
                            #ax_dist.set_ylim(-3.,3.,1)
                            ax_dist.set_ylim(-6.,6.,2)
                            xlim = ax_dist.get_xlim()
                            ax_dist.axhline(0., color='k', lw=.5)
                            ax_dist.set_xlim(xlim)
                            ax_dist.legend(loc='upper left', ncol=3)
                            
                            fig_dist.savefig(os.path.join(case_figs_dir, f'dist_{inv_label}_Xinv-Xsrc_all_forcings.png'), dpi=300, bbox_inches='tight')
                            fig_dist.show()
                    ...
                    # -------------------------------------------------------------
                    # Plotting the X-inv-filtred ??? of source data forcings X: GHG, AER and NAT                    
                    if plot_inv_filt_x :
                        ...
                        if len(Nbe_list) > 1:
                            print(f"\n *** too many inverted values {len(Nbe_list)} for plotting Xinv Plotted only in case of one HIST/Obs profile inverted ***")
                            print(f"Calcul de RMSE pour chaque forçage à partir de {rmse_start_year} à 2014 entre le forçage moyen des données source du modèle {mod_data_source} et le forçage moyen des données inversées filtrées({Xinv_arr.shape[1]} forçages).")
                            print(f"Calcul de RMSE pour chaque forçage à partir de {rmse_start_year} à 2014 entre le forçage moyen des données des autres modèles et le forçage moyen des données inversées filtrées({Xinv_arr.shape[1]} forçages).")
                            print(f"Calcul de RMSE pour chaque forçage à partir de {rmse_start_year} à 2014 entre le forçage moyen des données source du modèle {mod_data_source} et le forçage moyen des données des autres modèles.")
                            current_inv_years = inversion_years
                            for iforc,(forc,cs) in enumerate(zip(forcing_names[:3],forcing_colors[:3])) :
                                cs = gt.darker_color(cs)
                                if one_ax_by_forcing :
                                    print("ax INV-filt:",iforc)
                                    print("Xinv_arr.shape:",Xinv_arr.shape)
                                    ax = axes[iforc]
                                if forc == 'ghg':
                                    FORC_inv_array = signal.filtfilt(b_lp_filter, a_lp_filter, Xinv_arr[:,:,0,:]).mean(axis=0)
                                    FORC_src_from_model = GHG_mod_for_inv_array
                                    FORC_mean_oth_mod = GHG_ens_ab_arr_m
                                elif forc == 'aer':
                                    FORC_inv_array = signal.filtfilt(b_lp_filter, a_lp_filter, Xinv_arr[:,:,1,:]).mean(axis=0)
                                    FORC_src_from_model = AER_mod_for_inv_array
                                    FORC_mean_oth_mod = AER_ens_ab_arr_m
                                elif forc == 'nat':
                                    FORC_inv_array = signal.filtfilt(b_lp_filter, a_lp_filter, Xinv_arr[:,:,2,:]).mean(axis=0)
                                    FORC_src_from_model = NAT_mod_for_inv_array
                                    FORC_mean_oth_mod = NAT_ens_ab_arr_m
                                else:
                                    FORC_inv_array = None
                                ...
                                if FORC_inv_array is not None:
                                    ...
                                    rmse = gt.RMSE_mean(FORC_inv_array, FORC_src_from_model, n_end = lenDS_4_rms)
                                    print(f'RMSE for {forc} Xinv_filt / X_src: {rmse}')
                                    
                                    rmse = gt.RMSE_mean(FORC_inv_array, FORC_mean_oth_mod, n_end = lenDS_4_rms)
                                    print(f'RMSE for {forc} Xinv_filt / X_other_mod: {rmse}')
                                    
                                    rmse = gt.RMSE_mean(FORC_src_from_model, FORC_mean_oth_mod, n_end = lenDS_4_rms)
                                    print(f'RMSE for {forc} X_src / X_other_mod: {rmse}')
                                    ...
                                    if plot_forc_shaded_region :    
                                        if plot_inv_filt_forc_shaded_region :                                        
                                            if current_std_FORC_m is not None:
                                                ax.fill_between(current_inv_years, current_meanFORC_m - current_std_FORC_m, current_meanFORC_m + current_std_FORC_m,
                                                                ec=cs, fc=gt.lighter_color(cs), alpha=alpha_for_mean_shaded, linewidth=lw_for_mean_shaded,
                                                                hatch=hatch_for_mean_shaded_inv, zorder=2)
                                                ax.fill(np.NaN, np.NaN, 
                                                        ec=cs, fc=gt.lighter_color(cs), alpha=alpha_for_mean_shaded, linewidth=lw_for_mean_shaded,
                                                        hatch=hatch_for_mean_shaded_inv, label=f"inv {forc.upper()} filt")
                                        ax.plot(current_inv_years, current_meanFORC_m, color=cs, lw=1.5, ls='--', label=f"inv {forc.upper()} filt")
                                        
                                    else:
                                        ax.errorbar(current_inv_years, current_meanFORC_m, yerr=current_std_FORC_m, color=cs,
                                                    lw=2, elinewidth=1, alpha=0.8, label=f"inv {forc.upper()} filt")
                                        #hdf[iforc], = ax.plot(current_inv_years, current_meanFORC_m, color=c, lw=2, alpha=0.8)
                            ...
                            for iforc,(forc,cs) in enumerate(zip(forcing_names[:3],forcing_colors[:3])) :
                                ...
                                if one_ax_by_forcing :
                                    ...
                                    ax = axes[iforc]
                                if forc == 'ghg':
                                    FORC_inv_array = signal.filtfilt(b_lp_filter, a_lp_filter, Xinv_arr[0,:,0,:])
                                    FORC_src_from_model = GHG_mod_for_inv_array
                                    FORC_mean_oth_mod = GHG_ens_ab_arr_m
                                elif forc == 'aer':
                                    FORC_inv_array = signal.filtfilt(b_lp_filter, a_lp_filter, Xinv_arr[0,:,1,:])
                                    FORC_src_from_model = AER_mod_for_inv_array
                                    FORC_mean_oth_mod = AER_ens_ab_arr_m
                                elif forc == 'nat':
                                    FORC_inv_array = signal.filtfilt(b_lp_filter, a_lp_filter, Xinv_arr[0,:,2,:])
                                    FORC_src_from_model = NAT_mod_for_inv_array
                                    FORC_mean_oth_mod = NAT_ens_ab_arr_m
                                else:
                                    FORC_inv_array = None
                    ...
                    # X reference (or Background for inversion)
                    # -------------------------------------------------------------------------
                    if plot_mean_other_mod :
                        if one_ax_by_forcing :
                            ax = axes[0]  # GHG
                            tmp_plot_label = "$GHG_{ref.(other)}$"
                        else:
                            tmp_plot_label = "$FORC_{ref.(other)}$"
                        hfo = ax.plot(inversion_years, GHG_ens_ab_arr_m.T, c = 'black', #c=gt.darker_color(forcing_inv_colors[0]),
                                        ls='dashdot', lw=1.5, label=tmp_plot_label)
                        if one_ax_by_forcing :
                            ax = axes[1]  # AER
                            tmp_plot_label = "$AER_{ref.(other)}$"
                        else :
                            tmp_plot_label = None
                        ax.plot(inversion_years, AER_ens_ab_arr_m.T, c = 'black', #c=gt.darker_color(forcing_inv_colors[1]),
                                        ls='dashdot', lw=1.5, label=tmp_plot_label)
                        if one_ax_by_forcing :
                            ax = axes[2]  # NAT
                            tmp_plot_label = "$NAT_{ref.(other)}$"
                        else :
                            tmp_plot_label = None
                        ax.plot(inversion_years, NAT_ens_ab_arr_m.T, c = 'black', #c=gt.darker_color(forcing_inv_colors[2]),
                                        ls='dashdot', lw=1.5, label=tmp_plot_label)
                    ...
                    if False:
                        ...
                        for iforc,(forc,ci,x_mean,x_std,x_meanstd) in enumerate(zip(forcing_names[:3],forcing_inv_colors[:3],tmp_xarr_mean,tmp_xarr_std,tmp_xarr_meanstd)) :
                            ...
                            if plot_forc_shaded_region :
                                ...
                                if x_std is not None and False:
                    ...
                    if mod_to_invert in obs_set:
                        if one_ax_by_forcing :
                            ax = axes[3]   # pour HIST uniquement
                        HIST_label = "OBS"
                        ...
                        ############################################################################################################################################################################
                        # inversion_years est de meme shape que current_mean_ObsHIST_m
                        
                        print('current_mean_ObsHIST_m shape:',current_mean_ObsHIST_m.shape)
                        print('inversion_years shape:',inversion_years.shape)
                        current_mean_ObsHIST_m.shape = inversion_years.shape
                        print('current_mean_ObsHIST_m shape:',current_mean_ObsHIST_m.shape)
                        ...
                        if plot_hist2inv :
                            if one_ax_by_forcing :
                                ax = axes[3]
                            current_HIST_m = HIST_mod_for_inv_array[Nbe_list,:]
                            if len(Nbe_list) > 1:
                                current_mean_HIST_m = current_HIST_m.mean(axis=0)
                                current_std_HIST_m = local_std_coeff * current_HIST_m.std(axis=0,ddof=1)
                                
                                if plot_hist2inv_shaded_region :
                                    ax.fill_between(inversion_years, current_mean_HIST_m - current_std_HIST_m, current_mean_HIST_m + current_std_HIST_m,
                                                    ec=hist_color, fc=gt.lighter_color(hist_color, factor=0.5), alpha=alpha_for_shaded, linewidth=lw_for_shaded,
                                                    hatch=hatch_for_hist2inv_shaded, zorder=5)
                                    hid = ax.fill(np.NaN, np.NaN, 
                                                    ec=hist_color, fc=gt.lighter_color(hist_color, factor=0.5), alpha=alpha_for_shaded, linewidth=lw_for_shaded,
                                                    hatch=hatch_for_hist2inv_shaded, label=f"src {HIST_label} to inv ({current_HIST_m.shape[0]})")
                                    ax.plot(inversion_years, current_mean_HIST_m, color=hist_color, lw=1, ls='-', label=f"mean src {HIST_label} to inv ({current_HIST_m.shape[0]})", zorder=5)
                                else:
                                    #ax.errorbar(inversion_years, current_mean_HIST_m, yerr=current_std_HIST_m, color=hist_color, lw=1.5, ls='-.', label=f"src {HIST_label} to inv ({current_HIST_m.shape[0]}))", zorder=5)
                                    ax.plot(inversion_years, current_mean_HIST_m, color=hist_color, lw=1, ls='-', label=f"src {HIST_label} to inv ({current_HIST_m.shape[0]}))", zorder=5)

                            else:
                                ax.plot(inversion_years, current_HIST_m.T, color=hist_color, lw=1, ls='-', label=f"src {HIST_label} to inv ({current_HIST_m.shape[0]})", zorder=5)
                    ...
                    if Yinvmean_arr.shape[0] == 1:
                        ...
                        #tmp_y_src = HIST_mod_for_inv_array
                        # tmp_y_err = None
                    ...
                    print(f"Calcul de RMSE pour le profil historique à partir de {rmse_start_year} à 2014 entre le profil historique moyen des données source du modèle {mod_data_source} et le profil historique moyen des données inversées ({Yinvmean_arr.shape[0]} profils).")
                    
                    rmse = gt.RMSE_mean(tmp_y, HIST_mod_for_inv_array, n_end= lenDS_4_rms)
                    print(f'RMSE for HIST between: Yinv / Y_mod: {rmse}')
                    print('Nbe_list:', Nbe_list)
                    print('Yinvmean_arr.shape:', Yinvmean_arr.shape)
                    ...
                    if plot_hist_shaded_region :
                        if plot_inv_hist_shaded_region:
                            if plot_mean_err :
                        ...
                        ax.plot(inversion_years, tmp_y, color=hist_inv_color, lw=1.5, ls='-', label=f"inv {tmp_hist_label_name}")
                    ...
                    if one_ax_by_forcing :
                        for iforc,forc in enumerate(forcing_names) :
                            ax = axes[iforc]
                            xmin,xmax = ax.get_xlim()
                            ax.hlines(0, xmin=xmin, xmax=xmax, lw=0.5, ls='-', color='k')
                            ax.set_xlim([xmin,xmax])
                    ...
                    title_tmp_label = f"{mod_for_title_label}: Inv. of {current_nbe_inverted_label} profile(s)"+\
                                      f" ({nb_inverted_single_patt} {HIST_label} inv. w. {n_x_forc} X forc.)"+\
                                      (" each." if nb_inverted_single_patt > 1 else ".")
        
                    if one_ax_by_forcing :
                        ax = axes[0]  # titre sur la premiere ...
                        #for iforc,forc in enumerate(forcing_names) :
                            #ax = axes[iforc]
                    '''else:
                        ax.set_title(title_tmp_label)'''   # décommenter ça pour le titre 
            ...
            suptitle_label += f"{base_case_to_explore}\n{cnn_name_base} [Inv. Settings: {inv_label.split('_')[0]}]"
            #if one_ax_by_forcing :       suptitle_label += f"\n{title_tmp_label}"   # décommenter ça pour le titre 

            #plt.suptitle(suptitle_label, 
            #fig.suptitle(title_tmp_label, 
